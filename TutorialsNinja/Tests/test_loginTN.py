    ...
    def test_ValidLogin(self, username, password):
        ...
        wait.until(
            ec.visibility_of_element_located(
                (By.XPATH, "//li[@class='dropdown open']/child::ul/li/a[text()='Login']")
            )
        ).click()
        ...
        try:
            ...
            try:
                error = wait.until(
                    ec.visibility_of_element_located(
                        (By.XPATH, "//div[@id='account-login']/child::ul/following::div[@class='alert alert-danger alert-dismissible']")
                    )
                ).text
                log.info(error)
